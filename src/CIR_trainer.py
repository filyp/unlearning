...
class CirTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.init_res = get_metrics(model)
        ...
        for epoch in range(cfg.max_num_epochs):
            ...
            if epoch % cfg.get("pca_every_n", 1) == 0:
                ...
                self.act_to_collapse = get_projections(
                    self.acts_list, cfg.act_proj_num, cfg.cir_niter
                )
                self.grad_to_collapse = get_projections(
                    self.grads_list, cfg.grad_proj_num, cfg.cir_niter
                )

            # ! get metrics
            res = get_metrics(model)
            wandb.log(res)
            if res["wikitext_loss"] > self.init_res["wikitext_loss"] * cfg.get(
                "loss_budget", 1.01
            ):
                break
    ...
    def training_step(self, model, inputs):
        ...
        if cfg.get("retaining_rate", 0) > 0:
            ...
            with trim_layers(model, self.max_layer):
                output = model(**r_batch, output_hidden_states=True)
            loss = loss_fns.cb_retain(output, r_batch, cfg)
            loss.backward()
