...
big_thresh = init_wikitext_loss * 1.01
...
for run_name, run_data in ret_data.items():
    ...
    ax2.plot(accs[:30], color=color, linestyle=style)
    # print(accs[30], label)
...
ax1.set_ylabel("WMDP-Cyber Accuracy (%)")
...
ax1.text(big_thresh - 0.002, ax1.get_ylim()[1], "1% disr.", ha="left", va="bottom")
...
plot_path = repo_root() / f"paper/plots/{stem}_cyber.pdf"
